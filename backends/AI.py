...
class DeviceManager:
    ...
    def insert_or_update_device(self, device_data):
        ...
        status = 'online' if device_data.get('Active', False) else 'offline'
        ...
        if device_id:
            ...
            SET ip_address = %s, mac_address = %s, hostname = %s, 
                manufacturer = %s, device_type = %s, status = %s, updated_at = CURRENT_TIMESTAMP
            WHERE id = %s
            """
    ...
    def fetch_historical_data(self, device_id, limit=1000):
        ...
        query = """
        SELECT b.upload, b.download, b.created_at, 
               COALESCE(n.connection_count, 0) as connection_count,
               TIMESTAMPDIFF(MINUTE, n.first_seen, b.created_at) as active_time
        FROM bandwidth b
        LEFT JOIN (
            SELECT device_id, COUNT(*) as connection_count, MIN(created_at) as first_seen
            FROM connections
            WHERE device_id = %s
            GROUP BY device_id
        ) n ON b.device_id = n.device_id
        WHERE b.device_id = %s
        ORDER BY b.created_at DESC
        LIMIT %s
        """
        
        results = self.db.fetch_all(query, (device_id, device_id, limit))
        ...
        for i, (upload, download, timestamp, connection_count, active_time) in enumerate(results):
            ...
            data.append({
                ...
                'active_time': min(active_time, 43200)  # Cap at 30 days to prevent extreme values
            })
...
class MLManager:
    ...
    def analyze_traffic_pattern(self, data_point, device_history=None):
        """
        Analyze traffic patterns to determine probable cause of network activity
        
        Args:
            data_point: Current network data point
            device_history: Optional historical data for context
            
        Returns:
            Dictionary with probable cause analysis
        """
        ...
        hour_of_day = data_point.get('hour_of_day', datetime.now().hour)
        
        # Traffic pattern signatures with probability scores
        pattern_signatures = {
            'video_streaming': {
                'conditions': [
                    (download > 2000, 0.5),             # High download
                    (download_change > 1000, 0.3),      # Sudden download increase
                    (download > upload * 5, 0.2),       # Much higher download than upload
                    (1 <= hour_of_day <= 23, 0.1)       # Typical video streaming hours
                ],
                'description': 'Streaming video (Netflix, YouTube, etc.)',
                'bandwidth_profile': 'High download, low upload, sustained traffic'
            },
            'video_conferencing': {
                'conditions': [
                    (download > 500, 0.3),              # Moderate download
                    (upload > 300, 0.3),                # Moderate upload
                    (0.5 <= (upload / max(download, 1)) <= 2, 0.3),  # Balanced upload/download
                    (9 <= hour_of_day <= 18, 0.1)       # Business hours
                ],
                'description': 'Video conferencing (Zoom, Teams, etc.)',
                'bandwidth_profile': 'Balanced upload and download, consistent traffic'
            },
            'file_download': {
                'conditions': [
                    (download > 2000, 0.4),             # High download
                    (download_change > 1500, 0.4),      # Very sudden download increase
                    (upload < 100, 0.2)                 # Low upload
                ],
                'description': 'Large file download or software update',
                'bandwidth_profile': 'Very high download, minimal upload'
            },
            'file_upload': {
                'conditions': [
                    (upload > 1000, 0.5),               # High upload
                    (upload_change > 800, 0.3),         # Sudden upload increase
                    (upload > download * 3, 0.2)        # Much higher upload than download
                ],
                'description': 'File upload or cloud backup',
                'bandwidth_profile': 'High upload, low download'
            },
            'gaming': {
                'conditions': [
                    (50 <= download <= 500, 0.3),       # Moderate, consistent download
                    (20 <= upload <= 300, 0.3),         # Moderate, consistent upload
                    (abs(upload_change) < 50 and abs(download_change) < 100, 0.3),  # Stable traffic
                    (17 <= hour_of_day or hour_of_day <= 2, 0.1)  # Typical gaming hours
                ],
                'description': 'Online gaming',
                'bandwidth_profile': 'Stable moderate upload and download, low jitter'
            },
            'web_browsing': {
                'conditions': [
                    (download < 500, 0.3),              # Lower download
                    (upload < 100, 0.3),                # Low upload
                    (download_change > 0 and download_change < 400, 0.3)  # Sporadic download spikes
                ],
                'description': 'Web browsing or social media',
                'bandwidth_profile': 'Intermittent download spikes, low upload'
            },
            'p2p_activity': {
                'conditions': [
                    (download > 300 and upload > 300, 0.3),  # Balanced high traffic
                    (upload > 500, 0.4),                # High upload
                    (22 <= hour_of_day or hour_of_day <= 4, 0.1)  # Typical P2P hours
                ],
                'description': 'Peer-to-peer file sharing',
                'bandwidth_profile': 'High upload and download, sustained over time'
            },
            'system_backup': {
                'conditions': [
                    (upload > 800, 0.4),                # High upload
                    (upload > download * 4, 0.3),       # Much higher upload
                    (0 <= hour_of_day <= 6, 0.3)        # Overnight hours (typical for backups)
                ],
                'description': 'System or cloud backup',
                'bandwidth_profile': 'High sustained upload, typically during off-hours'
            },
            'software_update': {
                'conditions': [
                    (download > 1000, 0.4),             # High download
                    (upload < 100, 0.3),                # Low upload
                    (download_change > 900, 0.3)        # Sudden increase
                ],
                'description': 'Software or OS update',
                'bandwidth_profile': 'High download spike, minimal upload'
            },
            'malware_activity': {
                'conditions': [
                    (upload > 200 and upload_change > 150, 0.3),  # Unexpected upload
                    (upload > download * 2, 0.3),       # Upload dominates
                    (22 <= hour_of_day or hour_of_day <= 5, 0.2),  # Overnight hours
                    (device_history and any(h.get('upload', 0) < 50 for h in device_history[-5:]), 0.2)  # Recent low activity
                ],
                'description': 'Possible malware or unauthorized data exfiltration',
                'bandwidth_profile': 'Unusual upload patterns, especially during off-hours'
            }
        }
        
        # Calculate probability score for each pattern
        results = []
        for pattern_name, pattern_data in pattern_signatures.items():
            # Calculate weighted score
            score = 0
            applicable_weight = 0
            
            for condition, weight in pattern_data['conditions']:
                if condition:
                    score += weight
                    applicable_weight += weight
            
            # Normalize score if we have applicable conditions
            if applicable_weight > 0:
                normalized_score = score / applicable_weight
            else:
                normalized_score = 0
            ...
            results.append({
                ...
                'bandwidth_profile': pattern_data['bandwidth_profile']
            })
        
        # Sort by probability (highest first)
        results.sort(key=lambda x: x['probability'], reverse=True)
        
        # Get top 3 probable causes
        top_results = results[:3]
        
        # Format the output with percentages
        for result in top_results:
            result['probability_pct'] = f"{result['probability']*100:.1f}%"
        
        return {
            'probable_causes': top_results,
            'current_traffic': {
                'upload': upload,
                'download': download,
                'upload_change': upload_change,
    ...
    def classify_anomaly(self, data_point, anomaly_score, rf_model=None):
        ...
        if hasattr(self, 'device_history_cache') and data_point.get('device_id') in self.device_history_cache:
            device_history = self.device_history_cache[data_point.get('device_id')]
        
        # Analyze probable causes of the traffic pattern
        cause_analysis = self.analyze_traffic_pattern(data_point, device_history)
        ...
        # Include top 3 probable causes in the detailed information
        anomaly_info['detailed_causes'] = cause_analysis['probable_causes']
        
        return anomaly_info

    def fallback_anomaly_detection(self, data_point):
        """Simple threshold-based anomaly detection as fallback when ML is not available, with cause analysis"""
        # Extract values
        upload = data_point.get('upload', 0)
        download = data_point.get('download', 0)
        upload_change = data_point.get('upload_change', 0)
...
class ScheduleManager:
    ...
    def retrain_all_models(self):
        ...
        try:
            ...
            query = "SELECT id FROM networks WHERE status = 'online'"
    ...
    def cleanup_old_data(self):
        ...
        try:
            ...
            query = "DELETE FROM connections WHERE created_at < DATE_SUB(NOW(), INTERVAL 30 DAY)"
            result3 = self.monitor.db_manager.execute_and_commit(query)
